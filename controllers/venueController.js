...
async function getVenueOwnerDashboardStats(req, res) {
  ...
  try {
    ...
    const [upcomingBookingsResult] = await connection.query(
      upcomingQuery,
      upcomingParams
    );
    ...
    const [pendingBookingsResult] = await connection.query(
      pendingQuery,
      pendingParams
    );

    // Get recent bookings from both venue_reservations and events
    let venueBookingsQuery,
      eventBookingsQuery,
      bookingParams = [];
    ...
    const [venueBookingsResult] = await connection.query(
      venueBookingsQuery,
      bookingParams
    );
    const [eventBookingsResult] = await connection.query(
      eventBookingsQuery,
      bookingParams
    );
...
async function getVenueDetails(req, res) {
  ...
  try {
    ...
    try {
      ...
      if (
        venue.availability_info &&
        typeof venue.availability_info === "string"
      ) {
